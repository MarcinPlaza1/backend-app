...
const postResolvers = {
  Query: {
    posts: async (parent, args) => {
      ...
      return await PostModel.find().populate('author').sort(options.sort).skip(options.skip).limit(options.limit);
    },
    post: async (parent, { id }, { redisClient }) => {
      ...
      if (post) await redisClient.setEx(`post:${id}`, 3600, JSON.stringify(post));
  ...
  Post: {
    comments: async (parent, { limit = 10, offset = 0 }) => await CommentModel.find({ post: parent.id }).populate('author').skip(offset).limit(limit),
    likesCount: async (parent) => await LikeModel.countDocuments({ targetType: 'Post', targetId: parent.id }),
    likedByUser: async (parent, args, { user }) => {
      if (!user) return false;
      const like = await LikeModel.findOne({ user: user.id, targetType: 'Post', targetId: parent.id });
      return !!like;
    },
  },
};
